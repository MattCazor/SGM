...
void dessineBateau(int type, float x, float quaiLargeur, float quaiHauteur);
...
void gestionEvenement(EvenementGfx evenement)
{
    ...
    case Affichage:
        ...
        portLargeur = largeur * 0.7;                        // Le port occupe maintenant 70% de la largeur (côté gauche étendu)
        quaiLargeur = portLargeur * 0.85;                   // Chaque quai prend 85% de la largeur du port
        quaiHauteur = hauteur * 0.2;                        // Hauteur des quais (20% de la hauteur totale)
        quaiX = (portLargeur - quaiLargeur) / 2;            // Centré horizontalement
        espaceEntreQuais = (hauteur - 3 * quaiHauteur) / 4; // Distance égale entre les quais (4 espaces)
        xOffset = quaiX + quaiLargeur * 0.02f;
        ...
        {
            int nb_bateaux = place_dans_quai(Liste_quai->premier);
            for (int i = 0; i < nb_bateaux; i++)
            {
                dessineBateau(1, xOffset + (i * (quaiLargeur * 0.2f)), quaiLargeur * 0.55f, quaiHauteur);
            }
            nb_bateaux = place_dans_quai(Liste_quai->dernier);
            for (int i = 0; i < nb_bateaux; i++)
            {
                dessineBateau(4, xOffset + (i * (quaiLargeur * 0.15f)), quaiLargeur * 0.5f, quaiHauteur);
            }
            nb_bateaux = place_dans_quai(Liste_quai->premier->suivant);
            Navire *tmp = Liste_quai->premier->suivant->attente;
            for (int i = 0; i < nb_bateaux; i++)
            {
                if (tmp->type == MARCHANDISE)
                {
                    dessineBateau(2, xOffset + (i * (quaiLargeur * 0.3f)), quaiLargeur * 0.27f, quaiHauteur);
                }
                else
                {
                    dessineBateau(3, xOffset + (i * (quaiLargeur * 0.3f)), quaiLargeur * 0.27f, quaiHauteur);
                }
            }
        }
        break;
    ...
    case Temporisation:
        if (etatProgramme == SIMULATION)
        {
            ...
            quaiX = (portLargeur - quaiLargeur) / 2;            // Centré horizontalement
            espaceEntreQuais = (hauteur - 3 * quaiHauteur) / 4; // Distance égale entre les quais (4 espaces)
            xOffset = quaiX + quaiLargeur * 0.02f;            // Décalage horizontal pour le premier bateau
...
void dessineBateau(int type, float x, float quaiLargeur, float quaiHauteur)
{
    ...
    {
    case 1: // Plaisancier
        dessineBateauPassagers(x, (espaceEntreQuais * (1 + 0) + quaiHauteur * 0) + quaiHauteur / (quaiHauteur - 5), quaiLargeur, quaiHauteur);
        break;

    case 2: // Marchandises
        dessineBateauMarchandises(x, (espaceEntreQuais * (1 + 1) + quaiHauteur * 1) + quaiHauteur / (quaiHauteur - 5), quaiLargeur, quaiHauteur);
        break;

    case 3: // Pétrolier
        dessinePetrolier(x, (espaceEntreQuais * (1 + 1) + quaiHauteur * 1) + quaiHauteur / (quaiHauteur - 5), quaiLargeur, quaiHauteur);
        break;

    case 4: // Yacht
        dessineYacht(x, (espaceEntreQuais * (1 + 2) + quaiHauteur * 2) + quaiHauteur / (quaiHauteur - 5), quaiLargeur, quaiHauteur);
        break;

    default:
        printf("Type de bateau invalide : %d\n", type);
        break;
    }
}